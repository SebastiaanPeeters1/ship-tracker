...
export default function App() {
  ...
  const [dataUsage, setDataUsage] = useState(() => JSON.parse(localStorage.getItem('dataUsage') || '{"mb":0}'));
  ...
  useEffect(() => { localStorage.setItem('email', userEmail); }, [userEmail]);
  useEffect(() => { localStorage.setItem('dataUsage', JSON.stringify(dataUsage)); }, [dataUsage]);
  ...
  const fetchShips = useCallback(async () => {
    ...
    for (const apiKey of apiKeys) {
      try {
        const res = await fetch('https://api.allorigins.win/raw?url=' + encodeURIComponent('https://aisstream.io/v0/stream'), {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            APIkey: apiKey.key,
            BoundingBox: [{ lat: 90, lon: -180 }, { lat: -90, lon: 180 }]
          })
        });
  ...
  const styles = {
    ...
    input: { padding: '10px 12px', backgroundColor: '#0f1838', border: '1px solid #3d5a80', borderRadius: '6px', color: '#e8eef7', fontSize: '14px', fontFamily: 'inherit' }
  };
      ...
      <div style={styles.header}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          {['dashboard', 'zones', 'ships', 'alerts', 'settings', 'debug'].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} style={{ ...styles.btnSecondary, backgroundColor: activeTab === tab ? '#00d9ff' : '#1a2847', color: activeTab === tab ? '#0a1128' : '#e8eef7' }}>
              {tab === 'dashboard' && '📊'} {tab === 'zones' && '🗺️'} {tab === 'ships' && '🚢'} {tab === 'alerts' && `🔔(${alerts.length})`} {tab === 'settings' && '⚙️'} {tab === 'debug' && '🐛'}
            </button>
          ))}
        </div>
      </div>
      ...
      <div>
        {activeTab === 'dashboard' && (
          <div>
            ...
            <div style={styles.card}>
              ...
              {alerts.length === 0 ? <p style={{ color: '#8899bb' }}>No alerts yet</p> : alerts.slice(0, 10).map(a => (
        ...
        {activeTab === 'settings' && (
          <div>
            ...
            <div style={styles.card}>
              <h3 style={{ marginTop: 0, color: '#00d9ff' }}>📧 Email</h3>
        ...
        {activeTab === 'debug' && (
          <div>
            ...
            <div style={{ ...styles.card, borderColor: '#fbbf24' }}>
              <button onClick={() => { if (zones.length && ships.length) setAlerts(prev => [{ id: Date.now(), shipName: ships[0].name, zoneName: zones[0].name, timestamp: new Date(), imo: ships[0].imo, mmsi: ships[0].mmsi }, ...prev]); }} style={{ ...styles.btn, backgroundColor: '#fbbf24', color: '#0a1128' }}>🧪 Test Alert</button>
            </div>
            <div style={{ ...styles.card, borderColor: '#fbbf24' }}>
